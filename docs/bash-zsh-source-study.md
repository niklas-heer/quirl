...
Quirl's `Catalog::builtin()` remains the source for its own help, completion,
docs, and AI discovery, while Clap owns executable argument parsing. The
existing catalog/Clap parity tests are the correct control for Quirl builtins.
Strict KDL and its deterministic SQLite compiler cover a different domain:
curated metadata for external native executables. They do not generate or
replace Quirl's builtin declarations. Carapace may propose review-only KDL
drafts from a pinned revision, but it is never a runtime provider or schema
intermediary.
