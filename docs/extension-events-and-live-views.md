...
session-restore record. `quirl events validate trace.json` validates a complete
trace without loading an extension. Progress messages, output text, and
cancellation reasons containing terminal control bytes are rejected. Nested
annotation values are scanned iteratively under explicit node, depth, and text
budgets.
