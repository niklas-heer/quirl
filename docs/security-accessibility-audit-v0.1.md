...
| Accessibility | `auto` selects Ratatui only for a capable TTY; `simple`, non-TTY stderr, `TERM=dumb`, or height below five rows selects Reedline; `NO_COLOR` retains rich layout without color; dumb or non-UTF-8 terminals use ASCII automatically; private-use glyphs are automatic only for terminals with a documented bundled Nerd symbol fallback and otherwise require explicit opt-in; mode/editor state is textual; panels require a plain fallback; noninteractive structured output has no decoration | `explicit_simple_surface_always_degrades`; rich frame/status buffer tests; `terminal_styles_require_an_interactive_color_capable_terminal`; `auto_symbols_promote_only_with_unicode_and_bundled_nerd_font_evidence`; prompt injection/profile tests; panel fallback/control tests; manual capability checks below |
