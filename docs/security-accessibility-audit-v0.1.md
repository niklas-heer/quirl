...
Result: historical audit; release readiness was blocked at the time of this
record, and later candidates require their own exact automated and human review
...
Exact automated evidence in the historical P14 refresh belongs only to candidate
...
Evidence commit `05df4b09349394cfd34f24d514e0e005365d0ed8` is documentation
only and is not the measured artifact. Later implementation changes mean these
results are not evidence for the corrected code.
...
`aarch64-apple-darwin` Rust target. The release artifact passed all nine
programmatic real-PTY scenarios at 120×40 and the
security/accessibility-focused commands below.
...
| Accessibility | `auto` selects Ratatui only for a capable TTY; `simple`, non-TTY stderr, `TERM=dumb`, or height below five rows selects Reedline; `NO_COLOR` retains rich layout without color; dumb or non-UTF-8 terminals use ASCII automatically; private-use icons are automatic only for terminals with a documented bundled Nerd symbol fallback and otherwise require explicit opt-in; live input has an ASCII `>` or solid Unicode `❯`, while path/right/status/continuation chrome contains no thin Powerline chevrons; mode/editor state is textual; exact visible-frame selection makes transcript, path/Git context, input, right-side values, panels, and the status bar copyable; panels require a plain fallback; noninteractive structured output has no decoration | `explicit_simple_surface_always_degrades`; rich frame/status buffer tests; `mouse_drag_crosses_transcript_context_input_and_status_bar`; `terminal_styles_require_an_interactive_color_capable_terminal`; `auto_symbols_promote_only_with_unicode_and_bundled_nerd_font_evidence`; prompt injection/profile tests; panel fallback/control tests; manual capability checks below |
...
The historical P14 exact-candidate refresh additionally ran these focused
commands after the canonical gate and before any evidence document was edited:

```sh
cargo xtask rich-pty --binary target/release/quirl
...
selects the Reedline fallback with ASCII separators and a `>` live-input
marker. In a capable TTY, `NO_COLOR=1` keeps the Ratatui inline layout while
removing color styling; mode and editor state remain textual. Unicode and Nerd
profiles use the solid `❯` live-input marker, but path/right/status/continuation
chrome never uses thin Powerline chevrons. The piped command is noninteractive
and prints only `plain` plus a newline.

The historical P14 candidate's release binary passed these three smoke checks.
A Unicode-aware scan found zero active C0/C1 controls in all three outputs; the
piped result was exactly `plain\n`.
