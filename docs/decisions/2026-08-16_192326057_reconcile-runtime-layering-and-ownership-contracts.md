+++
schema_version = 1
id = "01M2XHZ6Z9746X4SH1P1PET9C7"
title = "Reconcile runtime layering and ownership contracts"
date = "2026-08-16"
status = "accepted"
tags = ["architecture", "runtime"]
supersedes = ["01M2XHZ6WRQNS9VTHY13TCBSBE", "01M2XHZ6WYP05WCZ6BXVKE4AMW"]
superseded_by = []
depends_on = ["01M2XHZ6Y69RDWA9BNV5SMG7Y8"]
related_to = ["01M2XHZ6XVXTAP30HMCW9N5TMV", "01M2XHZ6XFS5RD8YVCBGMGZQAD", "01M2XHZ6YYD5SQD5S3ZGS2J76Y"]
+++
- Decision owners: Quirl maintainers
- Supersedes: [ADR 0002](2026-08-15_192325976_enforce-one-way-crate-layering.md) and [ADR 0003](2026-08-15_192325982_preview-runtime-layers.md)
- Supersedes in part: the picker/core layering statement and protocol-change
  evidence in [ADR 0008](2026-08-15_192326011_freeze-public-protocols-with-owner-defined-descriptors.md)
- Reconciles: the Windows process boundary in
  [ADR 0006](2026-08-15_192325999_platform-process-and-recovery-boundaries.md)
- Extends: [ADR 0010](2026-08-16_192326022_freeze-the-1-0-release-scope-around-unix-and-explicit-dialec.md)
- Constrains: proposed [ADR 0014](2026-08-16_192326046_external-history-tools-integrate-through-a-bounded-provider.md)
...
At this decision's acceptance commit, `quirl-core::CommandRunner` still
performed shell execution and `quirl-process` called it for `cd` and `ls`. It
was not an accepted second executor. The process-lifecycle hardening change
completed its retirement in this order:

1. Replaced the process layer's `CommandRunner` delegation with process-owned
...
2. Routed every executable process capability through `NativeExecutor` or an
   injected process-owned `ProcessHost`; move or delete the core runner's
   external-spawn behavior and tests.
3. Removed the public `CommandRunner` export and implementation and core's
   `shlex` dependency after verifying that no core owner remained.

Each step preserved bounds and child cleanup and passed the focused process
gates. Native execution now has one owner and no release or security claim
relies on the removed unbounded legacy runner.
...
[ADR 0024](2026-08-18_192326111_compile-curated-native-command-specifications-from-kdl.md) keeps the accepted Quirl crate
graph unchanged while allowing the foundation `quirl-catalog` crate to own a
strict KDL parser and deterministic SQLite **artifact format** for curated
external native specifications. It adds no Quirl dependency edge and does not
give the foundation layer user-cache, discovery-worker, network, or composition
ownership. Native-catalog parse diagnostics follow this ADR's inert foundation
diagnostic exception; effect-owning consumers map them to `ShellError`.
