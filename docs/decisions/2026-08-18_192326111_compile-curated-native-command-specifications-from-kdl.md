+++
schema_version = 1
id = "01M2XHZ70ZJFSMYM0GJZDHNS3Z"
title = "Compile curated native command specifications from KDL"
date = "2026-08-18"
status = "accepted"
tags = ["shell"]
supersedes = []
superseded_by = []
depends_on = ["01M2XHZ6XNA29RS8DBD0MYCT9V", "01M2XHZ6Z9746X4SH1P1PET9C7", "01M2XHZ70A9T5C8NJBK1Y98MNF"]
related_to = []
+++
- Extends: [ADR 0007](2026-08-15_192326005_make-the-semantic-catalog-the-authoritative-command-contract.md),
  [ADR 0016](2026-08-16_192326057_reconcile-runtime-layering-and-ownership-contracts.md), and
  [ADR 0021](2026-08-17_192326090_sqlite-local-command-intelligence.md)
...
output bounds. Import produces one deterministic draft KDL file named for each
selected root command in the review workspace; provenance and the import
manifest identify its source ownership. It never writes directly to the
curated source set or release database. Splitting roots keeps additions, removals,
...
5. Promote a named first version with the non-overwriting helper, then edit and
   review it as curated KDL. Existing curated definitions are never overwritten
   by import or promotion. Run formatting, strict checking, deterministic
   compilation, reader validation, and tests.
...
import-carapace`, `cargo xtask catalog promote`, `cargo xtask catalog fmt`,
`cargo xtask catalog check`, and `cargo xtask catalog build` operations. Import requires an explicit local
