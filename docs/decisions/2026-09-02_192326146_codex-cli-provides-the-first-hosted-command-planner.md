+++
schema_version = 1
id = "01M2XHZ722X2MYCHW02Q72H3PR"
title = "Codex CLI provides the first hosted command planner"
date = "2026-09-02"
status = "accepted"
tags = ["tooling", "ai"]
supersedes = []
superseded_by = []
depends_on = ["01M2XHZ70A9T5C8NJBK1Y98MNF", "01M2XHZ716PDP04MF43V2HP1V3"]
related_to = []
+++
- Extends: [ADR 0021](2026-08-17_192326090_sqlite-local-command-intelligence.md) and
  [ADR 0025](2026-08-18_192326118_fine-tuned-command-retrieval-model.md)
...
item fails the turn even if user Codex configuration exposes one. The owned
app-server child and process group are terminated and reaped when the
...
Protocol writes share the request's cancellation and deadline with response
waits. A child that stops reading cannot hold the planner worker indefinitely;
cancelled or partial writes discard the connection. Unix pipes use nonblocking
I/O and explicit shutdown, so reader and writer cleanup does not depend on a
detached descendant releasing an inherited descriptor.
