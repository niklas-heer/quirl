...
## Exact-candidate evidence status

Candidate A is `7bf188344ca61798a3cd8657787eacb8ec26ef84` and the
measured release artifact SHA-256 is
`81cd33388cf610a7aac23a9781dbf2771b5dfb6b01b17522c2257cd3676d0ae6`.
The automated gate passed on actual local macOS hardware on 16 August 2026;
the exact environment, commands, raw aggregate measurements, and artifact
identity are in the [performance record](benchmarks/release-v1.0.md). Evidence
commit B records those facts and is not the measured artifact commit.

**Release readiness remains blocked.** No named human reviewer used an actual
terminal application for the macOS checklist, no actual Linux hardware or
Linux terminal application was available, and the real-terminal demo has not
received human review. Every corresponding box below remains unchecked. The
automated real-PTY harness is supporting evidence, not a human signoff.

## 1. Freeze the candidate

- [x] The worktree is clean and `main` is synchronized with its release remote.
- [x] `Cargo.lock`, `rust-toolchain.toml`, the release profile, version strings,
  generated SDK, protocol manifest, and user-facing status text describe the
  candidate being shipped.
- [x] The reviewed config descriptor is schema v3, its golden fingerprint
  matches the candidate, and v0/v1/v2-to-v3 migration tests pass.
- [x] The supported compatibility boundary is unchanged: C0/C1-core is native
...
- [x] Deferred and best-effort work is visible in the README and release notes.
...
- [x] `cargo xtask check` passes with the pinned Rust 1.88 toolchain.
- [x] The generated Lua SDK has no diff after `cargo xtask sdk`.
- [x] Website dependencies were installed with `npm ci --prefix website` and
...
- [x] The deterministic product tour passes against the candidate binary.
- [x] The release binary and benchmark harness are built together from the
  clean candidate.
...
Candidate A's release artifact also passed every automated scenario in
`scripts/check-rich-pty.py` on actual macOS hardware. That run covered rich
editing, completion, integrated runtime behavior, review regressions, native
job control, noninteractive dialect islands, suspend/resume, fallback
selection, and `NO_COLOR`. It used a programmatic 120×40 pseudo-terminal, not
a named graphical terminal application. Consequently none of the human,
cross-terminal, resize, remote-PTY, or Linux boxes in this section are checked.
...
- [x] The harness accepts the clean revision embedded independently in both
  ...
  digest.
- [x] All enforced PTY latency, first-prompt, binary-size, and bounded-history
  budgets pass, or the release stops with the miss preserved in the record.
- [x] The exact release binary is at or below the 10 MiB hard ceiling
...
- [x] Rich-surface draw and edit latency are measured on the selected
  candidate; `QUIRL_UI_TIMINGS=1` is diagnostic evidence, not a substitute for
  the enforcing PTY harness.
- [x] The release evidence names candidate A's exact revision, artifact digest,
...
- [x] The performance record and the binary intended for publication describe
