...
- Restricted Lua 5.5.1 runs configuration, scripts, and trusted-Lua plugin
...
- The semantic catalog, LSP, rich/simple terminal surfaces, process lifecycle,
  ...
  complete Quirl builtin-command contract. Strict KDL is the human-authored
  contract for curated external native specifications and compiles to a
...
**Command intelligence path:** declare Quirl builtins in `Catalog::builtin()` +
compile curated external KDL + discover attributable local facts → normalize
`CommandSpec` → index the versioned CLI cache → serve every consumer (prompt,
docs, LSP, AI, plugins).
...
| --- | --- | --- |
| Quirl builtin commands | `Catalog::builtin()` plus executable CLI parity tests | Exact and compile-time checked |
| Curated external native commands | Strict attributed KDL compiled to deterministic `QCNC` SQLite | Human-reviewed; platform-filtered; no runtime Carapace dependency |
...
Rust `///` comments document Rust APIs but do not provide runtime reflection.
Quirl builtin command prose and machine-readable facts therefore live in
`Catalog::builtin()`, checked against the executable CLI. Curated external
native prose lives only in strict attributed KDL and compiles deterministically
to SQLite; generated rows and Carapace drafts are not authoring sources. Script
docstrings and validated declarations project into the composed catalog with
...
- **Freshness:** Quirl builtin and package catalog generation belongs to
  compilation and package validation. Exact public `CommandSpec` records
  without summaries, argument docs, types, examples, effects, or error codes
  fail the quality gate. Curated external KDL has its own strict narrower schema,
  deterministic artifact checksum, provenance/license review, and runtime
  fallback contract; it does not invent missing exact facts.
...
Quirl’s interactive data notation is expression-oriented and gradually typed, but deliberately remains a focused shell surface. It is parseable, formattable, and lintable; general-purpose modules and scripts use Lua 5.5.1.
...
| --- | --- | --- | --- |
| Lua (5.4 selection evidence; 5.5.1 current runtime) | Recognizable syntax, tiny mature VM, established tooling and `mlua` bridge | Dynamic semantics require annotations, linting, schemas, boundary diagnostics | Accepted core extension language |
...
| --- | --- | --- |
| `.lua` | Pinned Lua 5.5.1 through `mlua`; generated annotations/bindings; restricted modules and explicit capabilities | Core commitment |
...
`quirl config web [file]` opens a polished local configuration app; `quirl config tui <file>` presents the same model in the terminal. `config.lua` is the source of truth. Browser and TUI are schema-backed views of that file—not separate stores. Omitting `file` for `web` defaults to the discovered `config.lua` (`QUIRL_CONFIG_DIR`, `XDG_CONFIG_HOME`, or `~/.config/quirl`). The web view refreshes from the file on each page load and checks the source again before saving; it does not claim a background file watcher.
...
| Concern | Contract |
| --- | --- |
| One language | Settings, prompt logic, keymaps, hooks, commands, extensions use Lua 5.5.1. `quirl.config` is ordinary Lua backed by generated annotations and authoritative Rust schemas. |
...
```sh
quirl config web             # local loopback web app against the discovered config.lua
quirl config tui ~/.config/quirl/config.lua  # same schema, terminal presentation
...
| Release binary | Track exact bytes; ≤5 MiB ideal; >8 MiB emits an advisory warning. No default project size ceiling; an explicit caller maximum remains enforceable. One MiB is exactly 1,048,576 bytes |
...
- **Release budgets are enforced.** A named-hardware PTY harness measures 101
  ...
  stripped release binary is measured against the 5 MiB ideal and warns above
...
- **One composed semantic command catalog:** completion, highlighting, docs,
  validation, AI discovery, and plugins consume one versioned `CommandSpec`
  graph. `Catalog::builtin()`, curated external KDL, and local discovery remain
  distinct truth/admission paths before composition.
...
2. Quirl pins vendored Lua 5.5.1. Runtime-version moves require a deliberate
   runtime, SDK, compatibility, and benchmark review rather than an automatic
   upgrade; ADR 0027 records the 5.5.1 review.
