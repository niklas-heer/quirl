...
| Prompt symbols | `auto` selects Nerd icons for Kitty, WezTerm, and Ghostty because those terminals document bundled symbol fallbacks; it otherwise chooses Unicode for a UTF-8 locale and ASCII as the final fallback. `plain`, `unicode`, and `nerd_font` remain explicit overrides, and `TERM=dumb` always wins with the plain profile. |
