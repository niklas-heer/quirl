...
| --- | --- | --- | --- |
| Normal `❯` | Bytes and processes; compatibility grammar; non-zero exits set status; familiar process control | `docker ps \| grep healthy`, `cargo test --workspace`, `false \|\| echo recovered` | `mode normal` (legacy: `mode command`); force external with `^ls` |
