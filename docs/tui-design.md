...
The current schema v4 changes only polished defaults: a compact banner,
automatic completion after one character, and `rust_version` in the right
prompt. Unversioned v0 and explicit v1/v2/v3 documents migrate to v4 before
validation, retaining the Tokyo Night theme default.
