...
If packaging succeeded, the separate `package-<target>` artifact is retained for
one day even when measurement fails. It is diagnostic evidence, not an approved
release: the failed job still blocks aggregation and publication. Compare the
executable inside that archive with the report's binary digest when investigating
a failure.
...
If the tap PR job fails after publication, the Quirl release remains valid.
Preserve the failed job's logs and retry the standalone Homebrew workflow
against the existing immutable tag:

```sh
gh workflow run homebrew.yml --repo niklas-heer/quirl --ref main -f tag=vVERSION
```

The workflow regenerates and checks the formula from the published manifest,
then requires the normal protected `homebrew-tap` environment review before
opening or updating its tap PR. Review that PR and the exact archive digests
before merging; do not push directly to the tap default branch. This recovery
does not rebuild or replace release artifacts or move the tag. The standalone
path succeeded for v0.2.0 in
[run 33990534633](https://github.com/niklas-heer/quirl/actions/runs/33990534633).
