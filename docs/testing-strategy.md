...
[ADR 0034](decisions/0034-release-binary-budget.md); historical measurements retain
...
The retained download-channel fixture waits for an actual stalled read before
cancelling it, and keeps the data sender open until it observes `Interrupted`.
Its five-second watchdog bounds test failure and cleanup; it does not measure
production HTTPS download latency or change the 25 ms cancellation poll.

Native release performance samples have one total PTY deadline (two seconds by
default), shared by readiness, input, cursor replies, frame stabilization, and
normal shutdown. After the latency endpoints, successful samples clear their
input and request EOF, requiring exit status zero before starting the next
sample against the shared fixture. Shutdown does not enter the latency metrics.
The harness uses nonblocking terminal I/O and performs no terminal writes during
forced cleanup. It stops on the first failed sample, preserves the requested sample
...
be diagnosed independently of the final JSON report. Ordered timing and shutdown
observations are limited to the first 101 successful samples.
...
can briefly inherit close-on-exec descriptors between fork and exec. Isolation
keeps immediate closure assertions meaningful without retrying away leaks. The
