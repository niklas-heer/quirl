...
deterministic build. Use `cargo xtask catalog import-carapace`, `cargo xtask
catalog promote --command <name>`, `cargo xtask catalog fmt`, `cargo xtask
catalog check`, and `cargo xtask catalog build`; the exact pin, checkout, and
review procedure is documented in [the catalog schema](catalog-schema.md).
