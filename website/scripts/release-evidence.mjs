...
// Release tooling owns RELEASE_NOTES.md and replaces it for each candidate.
// A historical record remains mandatory in its documentation projections, but
// must neither decorate another version's notes nor change their generated bytes.
// Read both bounded canonical headings rather than infer identity from a missing
// marker; malformed identities or a cross-version marker fail closed.
function releaseNotesUseRecord(contents, record) {
  const version = '(\\d+\\.\\d+\\.\\d+(?:[-+][0-9A-Za-z.-]+)?)';
  const notesVersion = extractSingleField(
    contents, new RegExp(`^# Quirl ${version}$`, 'gm'), 'release notes version',
  );
  const recordVersion = extractSingleField(
    record, new RegExp(`^# ${version} exact-candidate performance record$`, 'gm'), 'record version',
  );
  if (notesVersion === recordVersion) return true;
  if (contents.includes(projectionBegin) || contents.includes(projectionEnd)) {
    throw new Error(
      `release notes ${notesVersion} must not contain the ${recordVersion} evidence projection`,
    );
  }
  return false;
}
...
export function synchronizeProjectionFiles(repositoryRoot, metadata, checkMode) {
  ...
  for (const path of projectionPaths) {
    ...
    if (path === 'RELEASE_NOTES.md'
      && !releaseNotesUseRecord(contents, readBounded(join(repositoryRoot, evidenceSourcePath)))) {
      continue;
    }
    const expected = replaceProjection(contents, metadata, path);
