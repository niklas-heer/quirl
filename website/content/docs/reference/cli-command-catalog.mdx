...
description: "Generated builtin command contracts from Quirl; installed completion packs, plugins, and local discovery are excluded."
---
...
- `repository` — Positional `repository` declared by the builtin command signature.
- `--root` — Override the parent directory for this managed clone
...
- `ask|managed|off` — Positional `ask|managed|off` declared by the builtin command signature.
...
- `--root` — Override the root used for this resolution
