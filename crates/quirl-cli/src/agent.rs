...
fn read_agent_document(path: &Path) -> Result<Vec<u8>, ShellError> {
    read_regular_file(ReadFileOptions {
        path,
        bytes_max: MAX_AGENT_DOCUMENT_BYTES,
        context: "agent validation document",
        help: "Pass a readable JSON document generated by `quirl agent` in a regular file at or below 4 MiB",
