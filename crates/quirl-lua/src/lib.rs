...
pub struct PromptConfig {
    /// Visual symbol profile. `auto` uses Nerd icons only when the terminal
    /// documents a bundled fallback; `nerd_font` explicitly enables them elsewhere.
    pub symbols: String,
...
pub fn sdk_lua() -> String {
    ...
    output = output.replace(
        "Auto never assumes a patched font; nerd_font enables Powerline glyphs explicitly.",
