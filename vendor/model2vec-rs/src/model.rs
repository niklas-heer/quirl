...
use tokenizers::{models::ModelWrapper, Tokenizer};
...
impl StaticModel {
    ...
    fn compute_metadata(tokenizer: &Tokenizer) -> Result<(usize, Option<usize>)> {
        ...
        // Read the same model field without serializing the complete vocabulary,
        // normalizer, decoder, and processor graph into a temporary JSON value.
        // Unigram serializes `unk_id`, not `unk_token`; retain that distinction.
        let unk_token = match tokenizer.get_model() {
            ...
            ModelWrapper::BPE(model) => model.get_unk_token().as_deref(),
...
mod tests {
    ...
    // Preserve the old JSON-derived metadata as an independent compatibility
    // oracle in tests only. Production must not retain Tokenizer serialization.
    fn metadata_via_serialization(tokenizer: &tokenizers::Tokenizer) -> anyhow::Result<(usize, Option<usize>)> {
        let mut lengths: Vec<_> = tokenizer.get_vocab(false).keys().map(|token| token.len()).collect();
        lengths.sort_unstable();
        let median = lengths.get(lengths.len() / 2).copied().unwrap_or(1);
        let value = serde_json::to_value(tokenizer)?;
        let unknown = value
            .get("model")
        ...
        let id = unknown
            .map(|token| {
                tokenizer
                    .token_to_id(token)
                    .map(|id| id as usize)
                    .ok_or_else(|| anyhow::anyhow!("unk_token '{token}' not found in vocabulary"))
            })
            .transpose()?;
        Ok((median, id))
    }
    ...
    #[test]
    fn typed_metadata_matches_serialization_for_every_model_and_unknown_token_boundary() {
        use serde_json::json;
        use tokenizers::{models::ModelWrapper, Tokenizer};

        let mut fixtures = Vec::new();
        for kind in ["WordPiece", "WordLevel", "BPE"] {
            for (vocabulary, unknown) in [
                (json!({"[UNK]": 0, "é": 1, "ordinary": 2}), json!("[UNK]")),
                (json!({"": 0, "x": 1}), json!("")),
                (json!({"ordinary": 0}), json!("missing")),
                (json!({}), json!("missing")),
            ] {
                fixtures.push(json!({
                    "type": kind, "vocab": vocabulary, "unk_token": unknown,
                    "continuing_subword_prefix": "##", "max_input_chars_per_word": 100,
                    "merges": []
                }));
            }
        }
        // BPE may have no unknown token. Unigram exposes only an unknown ID,
        // which the previous metadata policy deliberately did not filter.
        fixtures.push(json!({"type": "BPE", "vocab": {"x": 0}, "merges": [], "unk_token": null}));
        fixtures.push(json!({"type": "BPE", "vocab": {}, "merges": []}));
        fixtures.push(
            json!({"type": "Unigram", "vocab": [["<unk>", 0.0], ["é", -1.0]], "unk_id": 0, "byte_fallback": false}),
        );
        fixtures.push(json!({"type": "Unigram", "vocab": [["x", 0.0]], "unk_id": null, "byte_fallback": false}));
        for fixture in fixtures {
            let model: ModelWrapper = serde_json::from_value(fixture.clone()).unwrap();
            let tokenizer = Tokenizer::new(model);
            assert_eq!(
                super::StaticModel::compute_metadata(&tokenizer).map_err(|error| error.to_string()),
                metadata_via_serialization(&tokenizer).map_err(|error| error.to_string()),
                "metadata changed for {fixture}"
            );
        }
    }
