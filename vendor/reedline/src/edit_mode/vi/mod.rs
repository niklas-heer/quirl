...
const SEQUENCE_CHARACTERS_MAX: usize = 64;
...
///
/// Quirl limits cached sequences to 64 characters, numeric counts and their
/// product to 1,024, and expanded edit actions (including dot replay) to 1,024.
/// Admission failures latch a typed error for [`EditMode::take_input_error`]
/// before any action or mode change from the rejected sequence is emitted.
pub struct Vi {
    cache: Vec<char>,
            ...
            Event::Key(KeyEvent {
                ...
                (ViMode::Normal | ViMode::Visual, modifier, KeyCode::Char(c)) => {
                    ...
                    if !self.cache.is_empty() || (binding.is_none() && is_typeable) {
                        // A key stream may leave a command incomplete forever.
                        // Admit before retaining the next scalar; normal counts
                        // are rejected earlier when their numeric budget trips.
                        if self.cache.len() == SEQUENCE_CHARACTERS_MAX {
                            self.input_error = Some(crate::input_limits::resource_limit_error(
                                "Vi sequence characters",
                                SEQUENCE_CHARACTERS_MAX,
                                self.cache.len().saturating_add(1),
                            ));
                            self.cache.clear();
                            return ReedlineEvent::None;
                        }
                        ...
                        if let Err(error) = res.checked_multiplier() {
