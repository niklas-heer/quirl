...
use crate::input_limits::INPUT_ACTIONS_MAX as ACTIONS_MAX;
...
impl ParsedViSequence {
    ...
    pub(super) fn checked_multiplier(&self) -> std::io::Result<usize> {
        let multiplier = self.multiplier.unwrap_or(1);
        let count = self.count.unwrap_or(1);
        let total = multiplier.checked_mul(count).unwrap_or(usize::MAX);
        let observed = total.max(multiplier).max(count);
        if observed > ACTIONS_MAX {
    ...
    fn apply_multiplier(
        &self,
        raw_events: Option<Vec<ReedlineOption>>,
    ) -> std::io::Result<ReedlineEvent> {
        let multiplier = self.checked_multiplier()?;
        let Some(raw_events) = raw_events else {
            return Ok(ReedlineEvent::None);
        };
        let events = raw_events
            .into_iter()
            .filter_map(ReedlineOption::into_reedline_event)
            .collect::<Vec<_>>();
        // Dot replay can contain a previously expanded sequence. Count its
        // actual bounded tree before cloning it, not only the new numeric count.
        let weight = crate::input_limits::event_weight(&events)?;
        let observed = weight.checked_mul(multiplier).unwrap_or(usize::MAX);
        if observed > ACTIONS_MAX {
            return Err(crate::input_limits::resource_limit_error(
                "Vi expanded actions",
                ACTIONS_MAX,
        ...
        if events.is_empty() || events.contains(&ReedlineEvent::None) {
            Ok(ReedlineEvent::None)
        } else {
            Ok(ReedlineEvent::Multiple(
                std::iter::repeat(events)
                    .take(multiplier)
                    .flatten()
                    .collect(),
            ))
    ...
    pub fn to_reedline_event(&self, vi_state: &mut Vi) -> ReedlineEvent {
        let (events, command) = match (&self.multiplier, &self.command, &self.count, &self.motion) {
            (_, Some(command), None, ParseResult::Incomplete) => (
                self.apply_multiplier(Some(command.to_reedline(vi_state))),
                Some(command),
            ),
        ...
        match events {
            ...
            Err(error) => {
                vi_state.input_error = Some(error);
                ReedlineEvent::None
            }
        }
    }
        ...
        Some(x) if x.is_ascii_digit() => {
            ...
            while let Some(&c) = input.peek() {
                if c.is_ascii_digit() {
                    ...
                    let _ = input.next();
                    // Overflow is an admission sentinel, never an executable
                    // multiplier: checked_multiplier rejects it before effects.
                    let digit = usize::try_from(c).unwrap_or(usize::MAX);
                    count = match count
                        .checked_mul(10)
