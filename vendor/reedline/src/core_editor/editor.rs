...
pub struct Editor {
    ...
    input_limit_violation: Option<usize>,
...
impl Editor {
    pub(crate) fn check_input_limit(&mut self) -> std::io::Result<()> {
        match self.input_limit_violation.take() {
            Some(bytes) => {
                self.line_buffer.take_limit_violation();
                Err(crate::input_limits::input_limit_error(bytes))
            }
            None => Ok(()),
        }
    }

    fn latch_input_limit(&mut self, bytes: usize) {
        self.input_limit_violation = Some(bytes);
        // History/menu callers may move the cursor after a failed setter. Keep
        // the restored buffer inert until the engine observes the error too.
        self.line_buffer.record_limit_violation(bytes);
    }

    fn reject_failed_edit(&mut self, previous: LineBuffer) -> bool {
        if let Some(bytes) = self.line_buffer.take_limit_violation() {
            self.line_buffer = previous;
            self.latch_input_limit(bytes);
            return true;
        }
        false
    }
    ...
    pub(crate) fn set_line_buffer(&mut self, line_buffer: LineBuffer, undo_behavior: UndoBehavior) {
        if self.input_limit_violation.is_some() {
            return;
        }
        let mut line_buffer = line_buffer;
        if let Some(bytes) = line_buffer.take_limit_violation() {
            self.latch_input_limit(bytes);
            return;
        }
    ...
    pub(crate) fn run_edit_command(&mut self, command: &EditCommand) {
        ...
        // Selection replacement may delete before attempting insertion. Keep one
        // bounded snapshot so rejection restores source and cursor atomically,
        // before any undo point is committed. Never clone the entire undo stack.
        let previous = self.line_buffer.clone();
        match command {
        ...
        if self.reject_failed_edit(previous) {
            return;
        }
    ...
    ///
    /// The callback edits a bounded candidate. If growth exceeds 65,536 bytes,
    /// its text, cursor and undo changes are discarded and the owning engine
    /// reports the latched input-limit error before submitting input.
        ...
        F: FnOnce(&mut LineBuffer),
    {
        ...
        let mut candidate = self.line_buffer.clone();
        func(&mut candidate);
        if let Some(bytes) = candidate.take_limit_violation() {
            self.latch_input_limit(bytes);
            return;
        }
        self.update_undo_state(undo_behavior);
        self.line_buffer = candidate;
    }
    ...
    fn replace_char(&mut self, character: char) {
        ...
        if let Some((start, end)) = self.get_selection() {
            ...
            // Count the replacement before allocating it: one ASCII grapheme
            // replaced by a four-byte scalar can otherwise allocate 4x the line.
            let replacement_bytes = self.line_buffer.get_buffer()[start..end]
                .graphemes(true)
                .map(|g| {
                    if g == "\n" || g == "\r\n" || g == "\r" {
                        g.len()
                ...
                .sum::<usize>();
            let projected_bytes = self.line_buffer.len() - (end - start) + replacement_bytes;
            if projected_bytes > crate::input_limits::INPUT_BYTES_MAX {
                self.line_buffer.record_limit_violation(projected_bytes);
                return;
            }
