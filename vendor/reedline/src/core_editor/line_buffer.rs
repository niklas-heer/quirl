...
///
/// Quirl bounds source to 65,536 UTF-8 bytes. Growth beyond that bound is
/// rejected without changing text or cursor, and latched for the owning editor
/// to report. No later operation can clear that rejection implicitly.
///
...
impl LineBuffer {
    // Cursor updates following a rejected replace must not use offsets computed
    // for text that was never inserted. The Editor consumes this sticky flag only
    // after restoring the whole pre-edit buffer; later edits cannot clear it.
    pub(crate) fn take_limit_violation(&mut self) -> Option<usize> {
        self.limit_violation.take()
    }

    pub(crate) fn record_limit_violation(&mut self, bytes: usize) {
        self.limit_violation = Some(bytes);
    }

    fn admit_length(&mut self, length: usize) -> bool {
        if self.limit_violation.is_some() {
            return false;
        }
        if length > crate::input_limits::INPUT_BYTES_MAX {
            self.limit_violation = Some(length);
            return false;
        }
        // Reserve the exact bounded delta, avoiding String's doubling policy.
        if length > self.lines.len() {
            self.lines.reserve_exact(length - self.lines.len());
        }
        true
    }
    ...
    /// Rejects more than 65,536 bytes under the type's latched-error contract.
    pub fn set_buffer(&mut self, buffer: String) {
        if !self.admit_length(buffer.len()) {
            return;
        }
        // A caller-owned String can have arbitrarily more capacity than content.
        // Copy only admitted bytes so that capacity never enters undo retention.
        self.lines = buffer.as_str().to_owned();
    ...
    /// Rejects growth beyond 65,536 bytes under the type's latched-error contract.
    pub fn insert_char(&mut self, c: char) {
        if !self.admit_length(self.lines.len().saturating_add(c.len_utf8())) {
            return;
        }
    ...
    ///
    /// Sets cursor to end of inserted string. Rejects growth beyond 65,536 bytes
    /// under the type's latched-error contract.
    ///
    /// ## Unicode safety:
    /// Does not validate the incoming string or the current cursor position
    pub fn insert_str(&mut self, string: &str) {
        if !self.admit_length(self.lines.len().saturating_add(string.len())) {
            return;
        }
        ...
        R: std::ops::RangeBounds<usize>,
    {
        ...
        // Invalid ranges remain programmer errors under this upstream API's
        // documented Unicode/range precondition, independently of byte admission.
        assert!(start <= end && end <= self.lines.len());
        let length = (self.lines.len() - (end - start)).saturating_add(replace_with.len());
        if !self.admit_length(length) {
            return;
        }
        self.lines.replace_range(start..end, replace_with);
    }
    ...
    pub fn swap_words(&mut self) {
        ...
        if word_1_range != word_2_range {
            ...
            // Replace the whole span once: swapping differently sized words
            // must not temporarily duplicate the larger one beyond admission.
            let mut replacement = String::with_capacity(word_2_range.end - word_1_range.start);
            replacement.push_str(&word_2);
            replacement.push_str(&insertion_line[word_1_range.end..word_2_range.start]);
            replacement.push_str(&word_1);
            self.replace_range(word_1_range.start..word_2_range.end, &replacement);
        }
    }
    ...
    pub fn swap_graphemes(&mut self) {
        ...
        if grapheme_1_start < updated_offset && grapheme_2_end > updated_offset {
            ...
            let mut replacement = String::with_capacity(grapheme_2_end - grapheme_1_start);
            replacement.push_str(&grapheme_2);
            replacement.push_str(&grapheme_1);
            self.replace_range(grapheme_1_start..grapheme_2_end, &replacement);
