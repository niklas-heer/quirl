    ...
    T: Default + Clone + Send,
{
    ...
    pub(super) fn insert(&mut self, value: T) {
        ...
        // Production entries are admitted LineBuffers of at most 64 KiB.
        // Evict before insertion: 128 entries retain at most 8 MiB of source,
        // including redo entries, plus bounded Vec/cursor metadata.
        if self.internal_list.len() == crate::input_limits::UNDO_STATES_MAX {
            self.internal_list.remove(0);
        }
        self.internal_list.push(value);
        self.index = self.internal_list.len() - 1;
    }
