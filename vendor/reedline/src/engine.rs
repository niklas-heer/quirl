...
// Continuous input must yield to editing/cancellation even when poll never
// reports an empty queue. The initial wait for a human is intentionally open;
// collection after that first event is bounded in bytes, records and wall time.
const INPUT_BATCH_DURATION: Duration = Duration::from_millis(20);

fn retain_input_event(
    ...
    let observed = text_bytes.saturating_add(event_bytes);
    if observed > crate::input_limits::INPUT_BATCH_TEXT_BYTES_MAX {
        return Err(crate::input_limits::resource_limit_error(
            "input batch text bytes",
            crate::input_limits::INPUT_BATCH_TEXT_BYTES_MAX,
            observed,
        ));
    }
    *text_bytes = observed;
...
impl Reedline {
    ...
    ///
    /// Source growth beyond 65,536 bytes returns an error classified by
    /// [`crate::is_input_limit_error`], without submitting input. Terminal modes
    /// are restored on interactive failure; a pending programmatic edit error
    /// is reported before entering raw mode.
    pub fn read_line(&mut self, prompt: &dyn Prompt) -> Result<Signal> {
        self.editor.check_input_limit()?;
        if let Some(error) = self.edit_mode.take_input_error() {
            return Err(error);
        }
        ...
        loop {
            ...
            if !self.immediately_accept {
                ...
                if needs_polling {
                    ...
                    retain_input_event(&mut events, &mut text_bytes, crossterm::event::read()?)?;
                }

                let collection_started = Instant::now();
                while events.len() < crate::input_limits::INPUT_ACTIONS_MAX
                    && collection_started.elapsed() < INPUT_BATCH_DURATION
                    && !completed(&events)
                        ...
                        .any(|event| matches!(event, Event::Resize(_, _)))
                {
                    ...
                    {
                        let remaining =
                            INPUT_BATCH_DURATION.saturating_sub(collection_started.elapsed());
                        if remaining.is_zero() || !event::poll(POLL_WAIT.min(remaining))? {
                            break;
                        }
                        retain_input_event(
                            &mut events,
    ...
    fn process_input_batch(
        ...
        // Parse and apply in arrival order. Parsing the entire batch first
        // changes Vi's mode before earlier insertion commands run, corrupting
        // rapid text+Esc input. Repaint remains deferred until the bounded batch
        // ends; admission rejects the current event before any of its effects.
        let mut resize = None;
        let mut action_budget = crate::input_limits::InputActionBudget::default();
        let mut need_repaint = false;
        for raw in events {
            let Ok(raw) = ReedlineRawEvent::try_from(raw) else {
                continue;
            };
            let event = self.edit_mode.parse_event(raw);
            if let Some(error) = self.edit_mode.take_input_error() {
                return Err(error);
            }
            if let ReedlineEvent::Resize(width, height) = event {
                resize = Some(ReedlineEvent::Resize(width, height));
                continue;
            }
            if let ControlFlow::Break(signal) =
                self.apply_batched_event(prompt, event, &mut action_budget, &mut need_repaint)?
            {
        ...
        if self.immediately_accept {
            if let ControlFlow::Break(signal) = self.apply_batched_event(
                prompt,
    ...
    fn apply_batched_event(
        &mut self,
        prompt: &dyn Prompt,
        event: ReedlineEvent,
        action_budget: &mut crate::input_limits::InputActionBudget,
        need_repaint: &mut bool,
    ) -> Result<ControlFlow<Signal>> {
        action_budget.admit(&event)?;
        self.editor.sync_edit_mode(self.edit_mode.edit_mode());
        let status = self.handle_event(prompt, event)?;
        self.editor.check_input_limit()?;
        match status {
            EventStatus::Exits(signal) => {
                ...
                Ok(ControlFlow::Break(signal))
            }
            EventStatus::Handled => {
                *need_repaint = true;
                Ok(ControlFlow::Continue(()))
            }
            EventStatus::Inapplicable => Ok(ControlFlow::Continue(())),
        }
    }
    ...
    ///
    /// Each command admits at most 65,536 source bytes. A command exceeding this
    /// bound preserves its preceding text, cursor and undo state; following
    /// commands are skipped until `read_line` reports the typed limit error
    /// (see [`crate::is_input_limit_error`]). Earlier successful commands in this
    /// slice remain committed. Undo retains at most 128 states (8 MiB of text).
    ...
    fn history_search_paint(&mut self, prompt: &dyn Prompt) -> Result<()> {
        ...
        if let HistoryNavigationQuery::SubstringSearch(substring) = navigation {
            ...
            let res_string = if self.use_ansi_coloring {
                ...
                crate::painting::escape_display_controls(&res_string).into_owned()
            };
