...
impl Screen {
    ...
    /// Returns the saturating number of primary scrollback rows evicted by its
    /// configured history capacity since the last full terminal reset.
    /// Quirl uses this read-only counter to make retained-output loss explicit.
    #[must_use]
    pub fn scrollback_rows_discarded(&self) -> u64 {
        self.grid.scrollback_rows_discarded()
    }
