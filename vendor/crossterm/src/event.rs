...
/// Whether an I/O error reports Quirl's bounded Unix terminal-input admission.
///
/// The caller should preserve this diagnostic and restore terminal state. The
/// rejected sequence is never delivered as a truncated key or paste event.
pub fn is_input_limit_error(error: &std::io::Error) -> bool {
    #[cfg(all(unix, feature = "use-dev-tty"))]
    { error.get_ref().is_some_and(|cause| cause.is::<source::unix::input_buffer::InputLimit>()) }
